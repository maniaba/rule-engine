...
final class FieldSelector
{
    ...
    public function __construct(private readonly array|object $data)
    {
    }
    ...
    private function resolveArray(array $array, string $filter): mixed
    {
        ...
        if (preg_match('/^(\w+):(>=|<=|>|<(?!>)|=|!=)?(.+)$/', $filter, $matches)) {
            ...
            $value = $this->castValue($matches[3]);
    ...
    private function evaluateCondition(mixed $itemValue, string $operator, mixed $value): bool
    {
