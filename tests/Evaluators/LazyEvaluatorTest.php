...
final class LazyEvaluatorTest extends TestCase
{
    public function testEvaluateEmptyRuleSet(): void
    {
        ...
        self::assertEmpty($results, 'Nema rezultata jer nema pravila');
        self::assertEmpty($evaluator->getFailedRules(), 'Nema padnutih pravila jer ih nema');
    }
    ...
    public function testEvaluateNoPassingRules(): void
    {
        ...
        self::assertCount(2, $failedRules, 'Oba pravila su pala');
        self::assertSame($failingRule1, $failedRules[0]);
        self::assertSame($failingRule2, $failedRules[1]);
    }
    ...
    public function testEvaluateStopsOnFirstPassingRule(): void
    {
        ...
        $failingRule->expects(self::never())->method('evaluate');
        ...
        self::assertCount(1, $results, 'Treba biti samo 1 rezultat evaluacije jer se prekida nakon uspješnog pravila');
        self::assertTrue($results[0]->result);

        self::assertEmpty($evaluator->getFailedRules(), 'Nema padnutih pravila');
    }
    ...
    public function testEvaluateStopsAtSecondRuleIfFirstFailsAndSecondPasses(): void
    {
        ...
        self::assertCount(2, $results, 'Evaluirano je 2 pravila, jer drugo uspije i prekida proces');
        self::assertFalse($results[0]->result);
        self::assertTrue($results[1]->result);

        $failedRules = $evaluator->getFailedRules();
        self::assertCount(1, $failedRules, 'Jedno pravilo je palo');
        self::assertSame($failingRule, $failedRules[0]);
    }
    ...
    public function testExecuteStopsOnFirstPassingRule(): void
    {
        ...
        $failingRule = $this->createMock(RuleInterface::class);
        $failingRule->method('evaluate')->willReturn(false);
        $failingRule->expects(self::never())->method('execute');

        // Passing rule - evaluira se i izvrsava, nakon čega se prekida izvršenje
        $passingRule = $this->createMock(RuleInterface::class);
        $passingRule->method('evaluate')->willReturn(true);
        $passingRule->expects(self::once())->method('execute')->with($context);

        // Treći rule se nikad ne bi trebao ni evaluirati ni izvršiti
        $thirdRule = $this->createMock(RuleInterface::class);
        $thirdRule->expects(self::never())->method('evaluate');
        $thirdRule->expects(self::never())->method('execute');
    ...
    public function testExecuteNoPassingRules(): void
    {
        ...
        $failingRule1 = $this->createMock(RuleInterface::class);
        $failingRule1->method('evaluate')->willReturn(false);
        $failingRule1->expects(self::never())->method('execute');

        $failingRule2 = $this->createMock(RuleInterface::class);
        $failingRule2->method('evaluate')->willReturn(false);
        $failingRule2->expects(self::never())->method('execute');
